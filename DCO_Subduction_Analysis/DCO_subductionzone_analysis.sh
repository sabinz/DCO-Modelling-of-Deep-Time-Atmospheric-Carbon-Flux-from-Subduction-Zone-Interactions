...
gmt_developement_directory='<GMT_Development_Build_Path_Required>'  
...
calculate_sz_length_carbonate(){
	...
	local prof_interval="44.7" # 44.7km spacing interval along the cross profile, with a 447km whisker length, 
	                           #there are a total of 10 units along the cross profile (447/44.7=10)
	local prof_length="894k" # Will search 447km from the subduction zone for cotinent feature. (894/2=447)
	...
	gmt grdmask reconstructed_carbonate_${age}.0Ma.gmt -Rd-180/180/-90/90 -I20k -N0/1/1  -G${carboante_mask_grid} -V 
...
calculate_sz_length_continentArc(){
	...
	# Define sampling distance, intervals and lengths of cross-profiles.
	local prof_spacing="10" # Cross profiles are spaced 10 km apart along the subduction zone
	local prof_interval="44.7" # 10 nodes along the cross profile are spaced 44.7 km apart along 447-km-long cross profile, 
	                           # (447/44.7=10)
	local prof_length="894k" # will search 447km from the subduction zone for continent polygon feature. (894/2=447)
	...
	local continent_mask_grid="reconstructed_carbonate_raster.nc"
	...
	gmt grdmask continental_polygons_closed.gmt -Rd-180/180/-90/90 -I300k -N0/1/1 -G${continent_mask_grid} -V
