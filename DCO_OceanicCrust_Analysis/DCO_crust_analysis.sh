...
validate_arguments(){
    ...
    do
        if [[ ! -f $i ]] || [[ $i != *.rot ]]; then
            echo >&2 "$rotfile: rotation file invalid type or does not exist"
            exit 1
        fi
    done
    ...
    do 
        if [[ ! -f $i ]] || [[ $i != *.gpml ]] && [[ $i != *.shp ]]; then
            echo >&2 "$topologies: topologies file invalid type or does not exist"
            exit 1
        fi
    done

    if  [ -z "$age_grid_direc" ] || [ ! -d "$age_grid_direc" ] || [ ! -d "$directory/$age_grid_direc" ]; then
    ...
    if [ -z "$sed_grid_direc" ] || [ ! -d "$sed_grid_direc" ] || [ ! -d "$directory/$sed_grid_direc" ]; then
